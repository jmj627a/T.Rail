...
namespace FreeNet
{
    ...
    //서버일 경우
    //-하나의 클라이언트 객체를 나타낸다.
    //-이인터페이스를 구현한 객체를 CNetworkService클래스의 session_created_callback호출 시 생성하여 리턴시켜 준다.
    //-객체를 풀링할지 여부는 사용자가 원하는 대로 구현한다.

    //클라이언트일 경우
    //-접속한 서버 객체를 나타낸다.

    //이 클래스의 모든 메소드는 thread unsafe하므로 공유자원에 접근할 때는 동기화 처리가 필요하다.
