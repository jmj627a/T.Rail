...
namespace FreeNet
{
    public class CUserToken
    {
        ...
        //패킷을 전송한다. 
        //큐가 비어있을 경우에는 큐에 추가한 뒤 바로 SendAsync매소드를 호출하고,
        ...
        public void process_send(SocketAsyncEventArgs e)
        {
            ...
            lock (this.cs_sending_queue)
            {
                //count가 0이하일 경우는 없겞지만.
                if (this.sending_queue.Count <= 0)
                {
                    throw new Exception("Sending queue count is lee than zero!");
                }
                ...
                //아직 전송하지 않은 대기중인 패킷이 있다면 ㄷㅏ시한번 전송을 요청한다.
                if (this.sending_queue.Count > 0)
                {
                    start_send();
                }
